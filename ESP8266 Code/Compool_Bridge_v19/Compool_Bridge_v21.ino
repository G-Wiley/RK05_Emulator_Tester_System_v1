...
// 1. designated as comments, the three lines of author/date/version above
// 2. added debugging statements inside of: "if (AnyDeviceChange()) {" at the bottom of "loop()"
// 3. fixed bug in "void prepare_server_response()" where debugging statements were displaying 
//    decimal values instead of hex values when outputting compool_devices_state to debugger 
// 4. commented out the debugger statement to print the cp status message in "void parse_message(int msg_byte)"
// 5. initialize mqtt_port[6] to "" instead of "1883"
// 6. increase #define max_mqtt_limit from 84 to 90
// 7. defined int temp_diff = 0; and int ptemp_diff = 0; to detect dithering temperatures
// 8. removed stoptimer code that was already commented out
// 9. created a new timer timer_temp_change_update to detect dithering temperatures
// 10. I worry about handleColor. It has two delay(1000) statements which will block other things from running,
//     e.g. compool serial link timing code, safe serial. Seems like it's not yet being called by the DH. 
// 11. /softreset handler added
...
char mqtt_server[40] = "";  // "192.168.0.203" change this to null string so WiFiManager parameter defaults to MQTT disabled
char mqtt_port[6] = "";
...
int pWater_Temperature = Water_Temperature; 
int pDesired_Pool_Temp = Desired_Pool_Temp, pDesired_Spa_Temp = Desired_Spa_Temp;
int temp_diff = 0;
int ptemp_diff = 0;
...
EventTimer timer_safe_after_serial_msg, timer_serial_msg_tx, timer_mqtt_keepalive, timer_intro_message, timer_temp_change_update;
...
// the delay(1000) statements will likely cause problems. Use a timer and a state variable instead.
void handleColor() {
...
boolean turn_on_device(int device_bitmask) {
  ...
  if(Service_Mode) {
    ...
    retval = false; // return false if we're in Service Mode and attempt to turn on the device
    return (retval);
  }
...
boolean turn_off_device(int device_bitmask){
  ...
  if(Service_Mode) {
    debugger("Compool in service mode");
    retval = false; // return false if we're in Service Mode and attempt to turn off the device
    return (retval);
  }
...
boolean AnyDeviceChange(){
  ...
  if(pcompool_devices_state != compool_devices_state){
    debugger("compool_devices_state changed from 0x" + String(pcompool_devices_state, HEX) + " to 0x" + String(compool_devices_state, HEX));
    retval = true;
  }
  if(pWater_Temperature != Water_Temperature) {
    ptemp_diff = temp_diff; //save previous temperature difference
    temp_diff = Water_Temperature - pWater_Temperature; // save present temperature difference
    // if the change is only one degree F and is in the opposite direction of the previous change then don't flag as a change
    // this is when the temperature is not ramping in a consistent direction
    if(((temp_diff == 1) && (ptemp_diff < 0)) || ((temp_diff == -1) && (ptemp_diff > 0))) { 
      // Start a timer so an update will eventually be sent if there are no further changes.
      // This timer is disabled in prepare_server_response(), which is called if an update is already sent for some other reason.
      // If there is no additional change for 2 hours then declare the last temp change as a change.
      timer_temp_change_update.starttimer(2*3600*1000, not_retriggerable);
      debugger("Water_Temperature direction change from " + String(pWater_Temperature) + " to " + String(Water_Temperature) +
        String(", diffs=") + String(temp_diff) + String(", ") + String(ptemp_diff));
    }
    else {
  ...
  if(timer_temp_change_update.processtimer()) { // if the timer expires then declare the last temp change as a change
    retval = true;
    debugger(String("timer_temp_change_update expired. A server update is being sent."));
  }
...
/********************************arduinoOTA*/
  ...
  SSDP.setName("ESP8266_Compool_Bridge");
...
void loop() 
{
  ...
  if(AnyDeviceChange()) {
    prepare_server_response();
    HTTPClient http;
    debugger(String("hubAddress = ") + hubAddress);
    debugger(String("http.begin = ") + String(http.begin("http://" + hubAddress + "/notify"))); // replaces the following /notify
    //http.begin("http://" + hubAddress + "/notify");
    http.addHeader("Content-Type", "application/x-www-form-urlencoded");
    debugger(String("jsonString length = ") + String(jsonString.length()));
    debugger(String("http.POST = ") + String(http.POST(jsonString))); // this replaces the following POST
