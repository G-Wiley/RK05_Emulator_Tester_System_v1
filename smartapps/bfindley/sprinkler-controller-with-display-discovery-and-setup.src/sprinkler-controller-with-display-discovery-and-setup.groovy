 ...
 */
definition(
    name: "Sprinkler Controller With Display Discovery and Setup",
    namespace: "bfindley",
    author: "bfindley",
	...
	if(canInstallLabs())
	{
		...
		return dynamicPage(name:"firstPage", title:"Discovery Started!", nextPage:"", refreshInterval: refreshInterval, install:true, uninstall: true) {
			section("Select a device...") {
				input "selectedSwitches", "enum", required:false, title:"Select Sprinkler Device \n(${switchesDiscovered.size() ?: 0} found)", multiple:true, options:switchesDiscovered
			}
...
//	unsubscribe()
    ...
    children.each{
    	if (it.getName() == "Sprinkler Controller with Display"){
    		bridgedevice = it
    	}
    }
	children.each{ 
...
def addSwitches() {
	...
	selectedSwitches.each { dni ->
		...
		if (!d) {
			...
			d = addChildDevice("bfindley", "Sprinkler Controller with Display", selectedSwitch.value.mac, selectedSwitch?.value.hub, [
					"label": selectedSwitch?.value?.name ?: "Sprinkler Controller with Display",
					"data": [
