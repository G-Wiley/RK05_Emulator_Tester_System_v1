 ...
 */
definition(
    name: "NewSprinkler Controller Discovery and Setup",
    namespace: "bfindley",
    author: "bfindley",
...
//	unsubscribe()
    ...
    children.each{
    	if (it.getName() == "Irrigation Controller 8 Zones v3.0.4"){
    		bridgedevice = it
    	}
    }
	children.each{ 
...
def addSwitches() {
	...
	selectedSwitches.each { dni ->
		...
		if (!d) {
			...
			d = addChildDevice("d8adrvn/smart_sprinkler", "Irrigation Controller 8 Zones v3.0.4", selectedSwitch.value.mac, selectedSwitch?.value.hub, [
					"label": selectedSwitch?.value?.name ?: "Irrigation Controller 8 Zones v3.0.4",
					"data": [
