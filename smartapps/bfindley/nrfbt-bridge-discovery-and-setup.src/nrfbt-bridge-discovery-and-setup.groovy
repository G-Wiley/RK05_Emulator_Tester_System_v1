 ...
 */
definition(
    name: "NRFBT Bridge Discovery and Setup",
    namespace: "bfindley",
    author: "bfindley",
    description: "Allows you to control your NRF/BT Windows with Smartthings",
...
preferences {
	page(name:"firstPage", title:"NRF/BT Bridge Setup", content:"firstPage")
    page(name: "addswitch")
...
def switchesDiscovered() {
	...
	def map = [:]
	switches.each {
		def value = it.value.name ?: "ESP8266 Switch ${it.value.ssdpUSN.split(':')[1][-3..-1]}"
...
//	unsubscribe()
    ...
    children.each{
    	if (it.getName() == "ESP8266_NRFBT_Bridge"){
    		bridgedevice = it
    	}
    }
	children.each{ 
...
def addSwitches() {
	...
	selectedSwitches.each { dni ->
		...
		if (!d) {
			...
			d = addChildDevice("bfindley", "ESP8266_NRFBT_Bridge", selectedSwitch.value.mac, selectedSwitch?.value.hub, [
					"label": selectedSwitch?.value?.name ?: "ESP8266_NRFBT_Bridge",
					"data": [
