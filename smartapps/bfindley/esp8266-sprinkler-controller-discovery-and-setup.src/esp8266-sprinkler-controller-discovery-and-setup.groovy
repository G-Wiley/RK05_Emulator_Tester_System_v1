 ...
 */
definition(
    name: "ESP8266 Sprinkler Controller Discovery and Setup",
    namespace: "bfindley",
    author: "bfindley",
...
//	unsubscribe()
    ...
    children.each{
    	if (it.getName() == "ESP8266_Sprinkler_Controller"){
    		bridgedevice = it
    	}
    }
	children.each{ 
...
def addSwitches() {
	...
	selectedSwitches.each { dni ->
		...
		if (!d) {
			...
			d = addChildDevice("bfindley", "ESP8266_Sprinkler_Controller", selectedSwitch.value.mac, selectedSwitch?.value.hub, [
					"label": selectedSwitch?.value?.name ?: "ESP8266_Sprinkler_Controller",
					"data": [
