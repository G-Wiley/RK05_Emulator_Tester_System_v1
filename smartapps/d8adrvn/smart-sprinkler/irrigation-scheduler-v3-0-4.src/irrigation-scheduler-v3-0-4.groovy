...
preferences {
    ...
    page(name: "sprinklerPage", title: "Sprinkler Controller Setup", nextPage: "weatherPage", uninstall: true) {
        ...
        section("Zone Times...") {
            input "zone1name", "string", title: "Zone 1 Name", description: "text", multiple: false, required: false
            input "zone1", "string", title: "Zone 1 Time", description: "seconds", multiple: false, required: false
            input "zone2name", "string", title: "Zone 2 Name", description: "text", multiple: false, required: false
            input "zone2", "string", title: "Zone 2 Time", description: "seconds", multiple: false, required: false
            input "zone3name", "string", title: "Zone 3 Name", description: "text", multiple: false, required: false
            input "zone3", "string", title: "Zone 3 Time", description: "seconds", multiple: false, required: false
            input "zone4name", "string", title: "Zone 4 Name", description: "text", multiple: false, required: false
            input "zone4", "string", title: "Zone 4 Time", description: "seconds", multiple: false, required: false
            input "zone5name", "string", title: "Zone 5 Name", description: "text", multiple: false, required: false
            input "zone5", "string", title: "Zone 5 Time", description: "seconds", multiple: false, required: false
            input "zone6name", "string", title: "Zone 6 Name", description: "text", multiple: false, required: false
            input "zone6", "string", title: "Zone 6 Time", description: "seconds", multiple: false, required: false
            input "zone7name", "string", title: "Zone 7 Name", description: "text", multiple: false, required: false
            input "zone7", "string", title: "Zone 7 Time", description: "seconds", multiple: false, required: false
            input "zone8name", "string", title: "Zone 8 Name", description: "text", multiple: false, required: false
            input "zone8", "string", title: "Zone 8 Time", description: "seconds", multiple: false, required: false
...
//        sendPush("TWC weather information for ${zipcode}: \n1 hour precip: ${getTwcConditions(zipcode).precip1Hour} inches. "
//        + "\n6 hour precip: ${getTwcConditions(zipcode).precip6Hour} inches.\n24 hour precip: ${getTwcConditions(zipcode).precip24Hour} inches. " 
//        + "\nForecast today: ${getTwcForecast(zipcode).daypart.qpf[0][0]} inches. \nForecast tomorrow: ${getTwcForecast(zipcode).daypart.qpf[0][1]} inches.")
...
def wasWetYesterday() {
    ...
    def yesterdaysPrecip = getTwcConditions(zipcode).precip24Hour 
    def yesterdaysInches= yesterdaysPrecip
...
def isWet() {
	...
	def forecastWeather = getTwcForecast(zip)
    def todaysPrecip = forecastWeather.qpf[0]
//    def todaysPrecip = getTwcForecast(zipcode).daypart.qpf[0][0]
...
def isStormy() {
    ...
    //String[] forecastPrecip=forecastWeather.forecast.simpleforecast.forecastday.qpf_allday.in?.toArray()
	def forecastWeather = getTwcForecast(zip)
    def forecastPrecip = forecastWeather.qpf[1]
//    def forecastPrecip = getTwcForecast(zipcode).daypart.qpf[0][1]
...
def isHot() {
	def forecastWeather = getTwcForecast(zip)
    def todaysHighTemp = forecastWeather.temperatureMax[0]
...
def water(attempts) {
    ...
    if(anyZoneTimes()) {
        ...
        for(int z = 1; z <= 24; z++) {
            ...
            if(zoneTime && zoneTime != "0") {
			String timestamp = new GregorianCalendar( 0, 0, 0, 0, 0, zoneTime.toInteger(), 0 ).time.format( 'mm:ss' )
            log.info(timestamp)
            	zoneTimes += "${z}:${zoneTime}"
            	log.info("Zone ${z} on for ${timestamp} minutes")
           	}
    	}
